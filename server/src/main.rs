...
use std::env;
// Зависимость стандартной библиотеки для работы с хэш-таблицами
...
use stor::{ServerStorage, UdpServerStorage}; // Внутренняя зависимость для работы с хранилищем принятых данных
use socket::{Packet, Socket}; // Внутренняя зависимость для работы с UDP-сокетом

mod stor; // Объявление модуля хранилища
mod socket; // Объявление модуля UDP-сокета

async fn process_packet(packet: Packet, storage: &UdpServerStorage, socket: &Socket, buf: &mut HashMap<Vec<u8>, Vec<u8>>) { // Функция для обработки пришедшего сообщения
    ...
    match message.clone() {
        Message::SendingReq(h) => { // Если пришел запрос на отправку
            if storage.can_save().await { // Если можно сохранить данные
        ...
        Message::RetrievingReq(h) => { // Если пришел запрос на получение
            if let Ok(d) = storage.get(&h).await { // Если данные с такой хэш-суммой представлены в хранилище
        ...
        Message::ContentFilled(h, d) => { // Если сообщение содержит данные
        ...
        Message::Empty(h) => { // Если сообщение-заглушка
...
async fn packet_handler(mut rx: broadcast::Receiver<Packet>, storage: &UdpServerStorage, socket: &Socket) { // Метод обработчика пакетов
...
async fn main() {
    ...
    for _ in 0..4 {
        ...
        tokio::spawn(async move { // Создание асинхронного потока
            packet_handler(rx, &storage_clone, &socket_clone).await; // Запуск обработчика пакетов в отдельном потоке
        });
    }

    tokio::spawn(async move { // Запуск асинхронного потока для приема пакетов от сокета
        socket.recv().await;
    });
