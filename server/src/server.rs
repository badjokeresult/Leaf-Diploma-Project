...
pub async fn run(mut shutdown_signal: oneshot::Receiver<()>) -> Result<(), Box<dyn Error>> {
    ...
    info!("Server was launched and started to work");
    ...
    // Graceful shutdown: освобождаем ресурсы
    info!("Starting to shut down...");
    recv_task.abort();
    let _ = handler.await;
    info!("Server was properly shutted down");
