...
impl Socket {
    pub async fn new() -> (Socket, mpsc::Receiver<Packet>) {
        let socket = Arc::new(UdpSocket::bind("0.0.0.0:62092").await.unwrap());
        socket.set_broadcast(true).unwrap();

        let (tx, rx) = mpsc::channel(100).await;
        (Socket {
            socket,
            sender: tx,
        }, rx)
    ...
    pub async fn send(&self, mut rx: mpsc::Receiver<Packet>) {
        loop {
            time::sleep(Duration::from_millis(100)).await;
            if let Some(p) = rx.recv().await {
                let (data, addr) = p.deconstruct();
