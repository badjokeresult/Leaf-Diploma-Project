use common::{
    Encryptor, Hasher, KuznechikEncryptor, ReedSolomonSecretSharer, SecretSharer, StreebogHasher,
};
...
pub trait Parts: Sized {
    ...
    async fn save_metadata(
        ...
        other: Self,
    ) -> Result<(), SavingMetadataError>;
    async fn load_from_metadata(
        path: impl AsRef<Path>,
    ) -> Result<(Self, Self), LoadingMetadataError>;
...
impl Parts for FileParts {
    ...
    async fn load_from_metadata(
        ...
        let ((data, recovery), (data_hashes, recovery_hashes)) = (
            parts.data.split_at(parts.data.len() / 2),
            parts.hashes.split_at(parts.hashes.len() / 2),
        );
    ...
    fn len(&self) -> Result<usize, Box<dyn std::error::Error>> {
        if self.data.len() == self.hashes.len() {
            return Ok(self.data.len());
        }
        Err(Box::new(ErrorKind::Deadlock.to_owned()))
    }
