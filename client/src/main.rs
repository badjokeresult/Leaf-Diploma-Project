...
async fn recv_file(
    ...
    for h in data_hashes.iter() {
        ...
        data.push(
            recv_chunk(&socket, h, block_size)
                .await
                .map_or(vec![0u8; block_size], |v| v), // Пытаемся получить данные из сети
    ...
    for i in 0..data.len() {
        if data[i].len() == 0 {
            ...
            recv.push(vec![0u8; block_size]); // Если текущий блок данных есть - возвращаем пустой вектор для индикации
        }
    }
